<template>
  <button
    class="share-button share-button--sms"
    type="button"
    :url="url"
    :btnText="btnText"
    :hasIcon="hasIcon"
...
export default {
  ...
  props: {
    ...
    btnText: { type: String, default: "SMS" },
    hasIcon: { type: Boolean, default: true }
  },
  methods: {
    openShareWindow() {
      eventEmit(this, "onShare", { name: "Sms" });
      const url = `sms:?&body=${encodeURIComponent(this.$props.url)}`;
...
$focus-color: hsla(0, 0%, 60%, 0.4);
$hover-color: hsla(0, 0%, 70%, 0.9);
$painted-color: hsl(0, 0%, 50%);
